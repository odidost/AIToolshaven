...
const codingArticlesComprehensive = [
  ...
  },
  {
    ...
    content: `
      ...
      <h2 class="text-2xl font-bold text-on-surface mt-10 mb-6 border-b border-outline pb-2">Pillar 4: Autonomous Test Loops & Verification</h2>
      <p class="mb-4">The defining factor separating brittle scripts from production-ready AI software engineers is automated closed-loop verification. When an agent creates a code change, it must prove that the application still works before asking for human approval.</p>
      
      <p class="mb-6">Modern agent frameworks use headless browser orchestration via Playwright and automated Vitest/Jest runners to simulate real user clicks, capture console stack traces, and iteratively refactor until all assertion tests return exit code 0.</p>
...
// Clean build of articles array
import { articles as currentArticles } from '../src/lib/articles';

const nonCodingArticles = currentArticles.filter(a => a.category !== 'Coding');

const combined = [
  ...nonCodingArticles.slice(0, 4), // 4 video articles
  ...codingArticlesComprehensive,    // 5 coding articles
  ...nonCodingArticles.slice(4)     // avatar + B2B articles + extras
];
...
console.log('Successfully written clean combined articles array with total count:', combined.length);
