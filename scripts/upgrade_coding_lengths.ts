...
// Let's replace the coding articles cleanly
let code = fs.readFileSync(file, 'utf8');

codingArticlesComprehensive.forEach(art => {
  const sIdx = code.indexOf(`slug: "${art.slug}"`);
  if (sIdx !== -1) {
    const blockStart = code.lastIndexOf('  {\n', sIdx);
    const blockEnd = code.indexOf('\n  },', sIdx);
    const formatted = `  {\n    title: ${JSON.stringify(art.title)},\n    category: ${JSON.stringify(art.category)},\n    slug: ${JSON.stringify(art.slug)},\n    date: ${JSON.stringify(art.date)},\n    readTime: ${JSON.stringify(art.readTime)},\n    author: ${JSON.stringify(art.author)},\n    summary: ${JSON.stringify(art.summary)},\n    imageUrl: ${JSON.stringify(art.imageUrl)},\n    content: \`\n${art.content.trim()}\n    \`\n  },`;
    code = code.substring(0, blockStart) + formatted + code.substring(blockEnd + 5);
  }
});

fs.writeFileSync(file, code, 'utf8');
console.log('Successfully updated all 5 coding articles with comprehensive 800-950+ word counts!');
