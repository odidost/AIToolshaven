...
import { VIDEO_MODELS, GenerationModel } from '@/data/generation-pricing';
import { Check, ArrowRight, TrendingDown, Info, ShieldAlert, Share2, Copy, Sparkles } from 'lucide-react';
...
export function VideoApiCalculator() {
  // Available models with developer API pricing
  const apiModels = useMemo(() => {
    return VIDEO_MODELS.filter(m => m.apiPricing.isAvailable);
  }, []);

  const [selectedModelId, setSelectedModelId] = useState<string>('runway-gen3-turbo');
  const [calculationMode, setCalculationMode] = useState<'total_volume' | 'per_user'>('total_volume');

  // Total Volume Mode
  const [monthlyVideos, setMonthlyVideos] = useState<number>(5000);

  // Per User Mode
  const [activeUsers, setActiveUsers] = useState<number>(500);
  const [videosPerUserMonth, setVideosPerUserMonth] = useState<number>(10);
  ...
  const [selectedDuration, setSelectedDuration] = useState<number>(
    selectedModel.apiPricing.defaultDuration || 5
  );

  const [selectedResolution, setSelectedResolution] = useState<string>(
    selectedModel.apiPricing.defaultResolution || '720p'
  );

  // Optional SaaS Unit Economics
  const [showSaasEconomics, setShowSaasEconomics] = useState<boolean>(false);
  const [subscriptionPrice, setSubscriptionPrice] = useState<string>('49');

  // Compute Total Monthly Videos
  const effectiveTotalVideos = useMemo(() => {
    if (calculationMode === 'per_user') {
      return Math.max(1, (activeUsers || 1) * (videosPerUserMonth || 1));
    }
    return Math.max(1, monthlyVideos || 1);
  }, [calculationMode, activeUsers, videosPerUserMonth, monthlyVideos]);
  ...
  const handleCopyScenario = useCallback(() => {
    ...
    url.searchParams.set('mode', 'video_api');
    url.searchParams.set('model', selectedModelId);
    url.searchParams.set('vol', String(effectiveTotalVideos));
    navigator.clipboard.writeText(url.toString());
    setCopiedScenario(true);
    setTimeout(() => setCopiedScenario(false), 2200);
  }, [selectedModelId, effectiveTotalVideos]);
  ...
  const currentCostPerVideo = useMemo(() => {
    if (selectedModel.apiPricing.billingUnit === 'per_generation') {
      return selectedModel.apiPricing.costPerUnit * resolutionMultiplier;
    }
    // per_second billing
    return selectedModel.apiPricing.costPerUnit * selectedDuration * resolutionMultiplier;
  }, [selectedModel, selectedDuration, resolutionMultiplier]);

  const currentMonthlyCost = effectiveTotalVideos * currentCostPerVideo;
  const currentAnnualCost = currentMonthlyCost * 12;
  const currentCostPerUser = calculationMode === 'per_user'
    ? currentCostPerVideo * videosPerUserMonth
  ...
  // Comparison across other video models
  const comparisonList = useMemo(() => {
    return apiModels.map(m => {
      const resMult = m.apiPricing.resolutionCostMultiplier
        ...
        : 1.0;
      let unitCost = 0;
      if (m.apiPricing.billingUnit === 'per_generation') {
        unitCost = m.apiPricing.costPerUnit * resMult;
      } else {
        unitCost = m.apiPricing.costPerUnit * selectedDuration * resMult;
      }
      const monthly = effectiveTotalVideos * unitCost;
      return {
        model: m,
        costPerVideo: unitCost,
        monthlyCost: monthly,
        isCheaper: monthly < currentMonthlyCost,
        saving: currentMonthlyCost - monthly
      };
    }).sort((a, b) => a.monthlyCost - b.monthlyCost);
  }, [apiModels, selectedResolution, selectedDuration, effectiveTotalVideos, currentMonthlyCost]);

  // Featured models for quick-access cards
  const featuredModels = useMemo(() => {
    const featuredIds = ['runway-gen3-turbo', 'kling-3-0-turbo', 'grok-imagine-video-1-5', 'google-veo-3-1-fast', 'seedance-2-5-pro', 'wan-2-1-pro'];
    return featuredIds.map(id => apiModels.find(m => m.id === id)).filter(Boolean) as GenerationModel[];
  }, [apiModels]);
    ...
    <div className="w-full space-y-8">
      ...
      <div className="space-y-1">
        <h3 className="text-xl sm:text-2xl font-black text-foreground tracking-tight flex items-center space-x-2">
          <span>🎬 Video API Cost</span>
        </h3>
        <p className="text-xs sm:text-sm text-on-surface-variant">
          Estimate what it will cost to power your video SaaS with AI generation APIs.
        </p>
      </div>
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-surface border border-border shadow-xs space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label htmlFor="video-api-model-select" className="block text-xs font-black text-on-surface-variant uppercase tracking-wider">
              Featured Video Generation Models
            </label>
            <span className="text-[11px] font-mono text-on-surface-variant">
              Showing 6 of {apiModels.length} API models
            </span>
          </div>
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (m.apiPricing.defaultDuration) setSelectedDuration(m.apiPricing.defaultDuration);
                    if (m.apiPricing.defaultResolution) setSelectedResolution(m.apiPricing.defaultResolution);
                  }}
                ...
                >
                  ...
                  <div className="mt-3 pt-2.5 border-t border-border/60 flex items-center justify-between">
                    <span className="text-xs font-mono font-bold text-foreground">
                      {m.apiPricing.billingUnit === 'per_generation'
                        ? `$${m.apiPricing.costPerUnit.toFixed(2)}`
                        : `$${m.apiPricing.costPerUnit.toFixed(3)}`}
                      <span className="text-[10px] text-on-surface-variant font-normal">
                        {m.apiPricing.billingUnit === 'per_generation' ? ' / gen' : ' / sec'}
                      </span>
                    </span>
                    {isSelected ? (
          ...
          <div className="pt-2 flex flex-wrap items-center gap-2">
            <span className="text-xs text-on-surface-variant font-medium">Or select from full catalog:</span>
            <select
              id="video-api-model-select"
              value={selectedModelId}
              onChange={(e) => {
                setSelectedModelId(e.target.value);
                const target = apiModels.find(m => m.id === e.target.value);
                if (target?.apiPricing.defaultDuration) setSelectedDuration(target.apiPricing.defaultDuration);
                if (target?.apiPricing.defaultResolution) setSelectedResolution(target.apiPricing.defaultResolution);
              }}
            ...
            >
              {apiModels.map(m => (
                <option key={m.id} value={m.id}>
                  {m.name} ({m.provider}) — {m.apiPricing.billingUnit === 'per_generation' ? `$${m.apiPricing.costPerUnit.toFixed(2)}/gen` : `$${m.apiPricing.costPerUnit.toFixed(3)}/s`}
                </option>
              ))}
            </select>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-border">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-2">
              {(selectedModel.apiPricing.supportedDurations || [5, 10]).map((dur) => (
                <button
                  key={dur}
                  type="button"
        ...
        <div className="pt-4 border-t border-border space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            {calculationMode === 'total_volume' && (
              <div className="flex items-center space-x-1.5">
                ...
                {[
                  { label: '1k (Starter)', val: 1000 },
                  { label: '5k (Growth)', val: 5000 },
                  { label: '25k (Scale)', val: 25000 },
                  { label: '100k (High)', val: 100000 },
                ].map((p) => (
                  <button
                    key={p.val}
                    type="button"
                    onClick={() => setMonthlyVideos(p.val)}
          ...
          {calculationMode === 'total_volume' ? (
            <div className="space-y-1.5 max-w-sm">
              <label htmlFor="total-monthly-videos-input" className="block text-xs font-bold text-foreground">
                How many videos will your SaaS generate each month?
              </label>
              <div className="relative">
                <input
                  id="total-monthly-videos-input"
                  type="number"
                  min="1"
                  step="500"
                  value={monthlyVideos}
                  onChange={(e) => setMonthlyVideos(Math.max(1, Number(e.target.value) || 1))}
                ...
                />
                <span className="absolute right-4 top-3 text-xs text-on-surface-variant font-medium">videos / mo</span>
              </div>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label htmlFor="video-active-users-input" className="block text-xs font-bold text-foreground">
                  Number of active users each month
                </label>
                <div className="relative">
                  <input
                    id="video-active-users-input"
                    type="number"
                    min="1"
                    value={activeUsers}
              ...
              <div className="space-y-1.5">
                <label htmlFor="video-per-user-input" className="block text-xs font-bold text-foreground">
                  How many videos does each user generate?
                </label>
                <div className="relative">
                  <input
                    id="video-per-user-input"
                    type="number"
                    min="1"
                    value={videosPerUserMonth}
                    onChange={(e) => setVideosPerUserMonth(Math.max(1, Number(e.target.value) || 1))}
                    className="w-full bg-surface-elevated border border-border rounded-xl px-4 py-3 text-base sm:text-sm font-bold text-foreground focus:outline-none focus:border-primary shadow-2xs"
                  />
                  <span className="absolute right-4 top-3 text-xs text-on-surface-variant font-medium">videos / user</span>
                </div>
              </div>
            </div>
          )}
      ...
      <div className="relative overflow-hidden p-6 sm:p-8 rounded-3xl bg-gradient-to-br from-surface via-surface-elevated/90 to-surface border border-primary/30 ring-1 ring-primary/20 shadow-xl space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-on-surface-variant block mb-1">
              Estimated Monthly Video API Spend ({selectedModel.name})
            </span>
          ...
          <div className="grid grid-cols-3 gap-3 text-xs font-mono bg-surface/80 backdrop-blur-md p-4 rounded-2xl border border-border shadow-xs">
            <div className="space-y-0.5">
              <span className="text-[10px] text-on-surface-variant uppercase tracking-wider block">Cost / Video</span>
              <strong className="text-foreground text-sm font-bold">{formatUnitCost(currentCostPerVideo)}</strong>
            </div>
        ...
        <div className="pt-3 border-t border-border">
          ...
          >
            <span>{showSaasEconomics ? 'Hide SaaS Gross Margin' : 'Forecast SaaS Gross Margin after video API spend &rarr;'}</span>
          </button>

          {showSaasEconomics && (
            <div className="mt-3 p-4 rounded-2xl bg-surface-elevated/80 border border-border space-y-3 text-xs">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <label htmlFor="video-sub-price-input" className="font-semibold text-foreground">
              ...
              {hasValidSub && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-border text-center font-mono">
                  ...
                  <div className="p-2 bg-surface rounded-xl border border-border">
                    <span className="text-[10px] text-on-surface-variant block">Video API Cost</span>
              ...
              <p className="text-[10px] text-on-surface-variant">
                Reflects direct AI video generation API spend. Excludes video transcoding, CDN bandwidth, and hosting infrastructure.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="space-y-4">
        <div>
          <h4 className="text-base font-bold text-foreground">
            Compare Video Generation APIs
          </h4>
          <p className="text-xs text-on-surface-variant">
            What this exact workload ({effectiveTotalVideos.toLocaleString()} &times; {selectedDuration}s videos/month) costs across other video models.
          </p>
        </div>
            ...
            >
              <div>
                ...
                <div className="text-xs text-on-surface-variant mt-1 font-mono">
                  {formatUnitCost(item.costPerVideo)} per {selectedDuration}s video
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-border flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (item.model.apiPricing.defaultDuration) setSelectedDuration(item.model.apiPricing.defaultDuration);
                  }}
