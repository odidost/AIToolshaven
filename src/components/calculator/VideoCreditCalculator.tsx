...
export function VideoCreditCalculator() {
  // Models with consumer credit pricing
  const creditModels = useMemo(() => {
    return VIDEO_MODELS.filter(m => m.consumerPricing.isAvailable);
  }, []);

  const [selectedModelId, setSelectedModelId] = useState<string>('kling-3-0-turbo');
  ...
  const handleCopyScenario = useCallback(() => {
    ...
    url.searchParams.set('mode', 'video_credits');
  ...
  // Mode A: Credits -> Videos
  const [availableCredits, setAvailableCredits] = useState<number>(3000);

  // Mode B: Videos -> Credits
  const [targetVideos, setTargetVideos] = useState<number>(50);
  ...
  // Credits per video calculation
  const creditsPerVideo = useMemo(() => {
    const baseCredits = selectedModel.consumerPricing.creditsPerGeneration;
    const durMult = selectedModel.consumerPricing.durationCostMultiplier
      ? (selectedModel.consumerPricing.durationCostMultiplier[selectedDuration] || (selectedDuration / 5))
      : (selectedDuration / 5);
    return Math.round(baseCredits * durMult);
  }, [selectedModel, selectedDuration]);

  const creditsPerSecond = selectedDuration > 0
    ? (creditsPerVideo / selectedDuration)
    : 0;

  // Mode A calculations (Credits -> Videos)
  const estimatedVideos = creditsPerVideo > 0
    ? Math.floor(Math.max(0, availableCredits) / creditsPerVideo)
    : 0;
  const creditsUsed = estimatedVideos * creditsPerVideo;
  const remainingCredits = Math.max(0, availableCredits - creditsUsed);

  // Mode B calculations (Videos -> Credits)
  const totalCreditsNeeded = Math.max(0, targetVideos) * creditsPerVideo;
  ...
  const effectiveCostPerVideo = matchingPlan && matchingPlan.monthlyCredits > 0
    ? (matchingPlan.monthlyPrice / (matchingPlan.monthlyCredits / creditsPerVideo))
    : null;

  // Model comparison across same credit budget
  const comparisonList = useMemo(() => {
    return creditModels.map(m => {
      const durMult = m.consumerPricing.durationCostMultiplier
        ? (m.consumerPricing.durationCostMultiplier[selectedDuration] || (selectedDuration / 5))
        : (selectedDuration / 5);
      const cpv = Math.round(m.consumerPricing.creditsPerGeneration * durMult);
      const possible = cpv > 0 ? Math.floor(Math.max(0, availableCredits) / cpv) : 0;
      return {
        model: m,
        creditsPerVideo: cpv,
  ...
  }, [creditModels, selectedDuration, availableCredits]);
    ...
    <div className="w-full space-y-8">
      ...
      <div className="space-y-1">
        <h3 className="text-xl sm:text-2xl font-black text-foreground tracking-tight flex items-center space-x-2">
          <span>🎬 Video Credit &amp; Generation Calculator</span>
        </h3>
        <p className="text-xs sm:text-sm text-on-surface-variant">
          Calculate how many videos your credits can generate across Kling, Runway, Luma, and other platforms.
        </p>
      </div>
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-surface border border-border shadow-xs space-y-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="inline-flex rounded-xl bg-surface-elevated p-1 border border-border text-xs font-bold">
            ...
            >
              I Have a Video Goal &rarr; Credits
            </button>
          </div>
        </div>
        ...
        <div className="space-y-3 pt-2 border-t border-border">
          <div className="flex items-center justify-between">
            <label htmlFor="video-credit-model-select" className="block text-xs font-black text-on-surface-variant uppercase tracking-wider">
              Featured Video Generation Platforms
            </label>
            <span className="text-[11px] font-mono text-on-surface-variant">
              Showing 6 of {creditModels.length} platforms
            </span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {creditModels
              .filter(m => ['google-veo-3-1-quality', 'grok-imagine-video-1-5', 'kling-3-0-turbo', 'seedance-2-5-pro', 'runway-gen3-turbo', 'wan-2-1-pro'].includes(m.id))
              .map((m) => {
                    ...
                    <div className="mt-3 pt-2.5 border-t border-border/60 flex items-center justify-between">
                      <span className="text-xs font-mono font-bold text-foreground">
                        ...
                        <span className="text-[10px] text-on-surface-variant font-normal">
                          {m.consumerPricing.creditUnit} / 5s
                        </span>
                      </span>
                      {isSelected ? (
          ...
          <div className="pt-2 flex flex-wrap items-center gap-2">
            <span className="text-xs text-on-surface-variant font-medium">Or select from all platforms ({creditModels.length} tools):</span>
            <select
              id="video-credit-model-select"
            ...
            >
              {creditModels.map(m => (
                <option key={m.id} value={m.id}>
                  {m.name} ({m.provider}) — {m.consumerPricing.creditsPerGeneration} {m.consumerPricing.creditUnit}/5s
                </option>
              ))}
            </select>
          </div>
          ...
          {calculationDirection === 'credits_to_output' ? (
            <div className="space-y-2">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <label htmlFor="available-video-credits-input" className="block text-xs font-bold text-foreground">
                ...
                <div className="flex items-center space-x-1.5">
                  ...
                  {[
                    { label: '600 (Basic)', val: 600 },
                    { label: '1,500 (Pro)', val: 1500 },
                    { label: '3,000 (Studio)', val: 3000 },
                    { label: '10,000 (Scale)', val: 10000 },
                  ].map((p) => (
                    <button
                      key={p.val}
                      type="button"
              ...
              <div className="relative max-w-sm">
                <input
                  id="available-video-credits-input"
                  type="number"
                  min="0"
                  step="100"
            ...
            <div className="space-y-2">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <label htmlFor="target-videos-input" className="block text-xs font-bold text-foreground">
                  How many {selectedDuration}s videos do you want to generate?
                </label>
                <div className="flex items-center space-x-1.5">
                  <span className="text-[11px] font-mono text-on-surface-variant hidden sm:inline">Presets:</span>
                  {[
                    { label: '10 (Quick Reel)', val: 10 },
                    { label: '50 (Batch)', val: 50 },
                    { label: '200 (Campaign)', val: 200 },
                    { label: '1,000 (Agency)', val: 1000 },
                  ].map((p) => (
                    <button
                      key={p.val}
                      type="button"
                      onClick={() => setTargetVideos(p.val)}
                      className={`px-2 py-0.5 rounded-lg text-[11px] font-mono transition-all cursor-pointer border ${
                        targetVideos === p.val
                          ? 'bg-accent/15 border-accent text-accent font-bold'
                          : 'bg-surface-elevated border-border text-on-surface-variant hover:text-foreground'
              ...
              <div className="relative max-w-sm">
                <input
                  id="target-videos-input"
                  type="number"
                  min="1"
                  step="10"
                  value={targetVideos}
                  onChange={(e) => setTargetVideos(Math.max(1, Number(e.target.value) || 1))}
                  className="w-full bg-surface-elevated border border-border rounded-xl px-4 py-3 text-base sm:text-sm font-bold text-foreground focus:outline-none focus:border-accent shadow-2xs"
                />
                <span className="absolute right-4 top-3 text-xs text-on-surface-variant font-medium">videos</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border/60 pb-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-[11px] font-mono font-bold tracking-wider uppercase text-accent">
              LIVE GENERATION CAPACITY FORECAST
            </span>
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex items-baseline space-x-3">
                <span className="text-4xl sm:text-5xl lg:text-6xl font-black text-foreground font-mono tracking-tight">
                  {estimatedVideos.toLocaleString()}
                </span>
                <span className="text-sm font-semibold text-on-surface-variant">
                  {selectedDuration}s videos
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono bg-surface/80 backdrop-blur-md p-4 rounded-2xl border border-border shadow-xs">
              <div className="space-y-0.5">
                <span className="text-[10px] text-on-surface-variant uppercase tracking-wider block">Rate / Video</span>
                <strong className="text-foreground text-sm font-bold">{creditsPerVideo} {selectedModel.consumerPricing.creditUnit}</strong>
              </div>
              ...
              <div className="space-y-0.5 border-l border-border/80 pl-3">
                ...
                <strong className="text-emerald-600 dark:text-emerald-400 text-sm font-bold">
                  {effectiveCostPerVideo ? `~${formatCurrency(effectiveCostPerVideo)}/vid` : '—'}
                </strong>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs font-mono bg-surface/80 backdrop-blur-md p-4 rounded-2xl border border-border shadow-xs">
              <div className="space-y-0.5">
                ...
                <strong className="text-foreground text-sm font-bold">{targetVideos.toLocaleString()} videos ({selectedDuration}s each)</strong>
              </div>
              <div className="space-y-0.5 border-x border-border/80 px-3">
                <span className="text-[10px] text-on-surface-variant uppercase tracking-wider block">Rate per Video</span>
                <strong className="text-foreground text-sm font-bold">{creditsPerVideo} {selectedModel.consumerPricing.creditUnit}</strong>
              </div>
      ...
      <div className="space-y-4">
        <div>
          <h4 className="text-base font-bold text-foreground">
            What gives you the most {selectedDuration}s videos for your credits?
          </h4>
          <p className="text-xs text-on-surface-variant">
            Generation capacity across video models for {availableCredits.toLocaleString()} credits.
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="border-b border-border bg-surface-elevated text-foreground font-bold">
                ...
                <th className="py-3 px-4">Credits / {selectedDuration}s Video</th>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 font-mono text-on-surface-variant">
                      {item.creditsPerVideo} {item.model.consumerPricing.creditUnit}
                    </td>
                    <td className="py-3 px-4 text-right font-mono font-bold text-foreground">
                      ~{item.possibleGenerations.toLocaleString()} videos
                    </td>
