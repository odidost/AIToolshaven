...
export function WorkflowForm({ initialData, workflowSlug }: { initialData: any, workflowSlug: string }) {
  ...
  const onSubmit = async (data: WorkflowFormValues) => {
    ...
    try {
      const result = await saveWorkflow(workflowSlug, data);
      toast.dismiss();
      if (result.success) {
        toast.success("Workflow saved successfully");
        if (workflowSlug === "new") {
          router.push(`/admin/cms/workflows/${result.slug}`);
        }
      } else {
      ...
      <form onSubmit={form.handleSubmit(onSubmit as any)} className="space-y-8">
        ...
        <div className="flex items-center justify-between bg-white p-4 border rounded-lg sticky top-14 z-10 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold">{initialData ? initialData.title : "New Workflow"}</h1>
          ...
          <div className="flex items-center gap-3">
            {initialData && (
              <a href={`/workflows/${initialData.slug}`} target="_blank" rel="noreferrer">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="e.g. Content Creation Workflow" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="content-creation" {...field} />
                      </FormControl>
              ...
              <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="e.g. account_tree" {...field} />
                      </FormControl>
                      <FormDescription>Material Symbols icon name</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Controls the visibility of this workflow on the site.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
