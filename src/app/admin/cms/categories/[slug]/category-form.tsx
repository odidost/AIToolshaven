...
export function CategoryForm({ initialData, categorySlug }: { initialData: any, categorySlug: string }) {
  ...
  const onSubmit = async (data: CategoryFormValues) => {
    ...
    try {
      const result = await saveCategory(categorySlug, data);
      toast.dismiss();
      if (result.success) {
        toast.success("Category saved successfully");
        if (categorySlug === "new") {
          router.push(`/admin/cms/categories/${result.slug}`);
        }
      } else {
      ...
      <form onSubmit={form.handleSubmit(onSubmit as any)} className="space-y-8">
        ...
        <div className="flex items-center justify-between bg-white p-4 border rounded-lg sticky top-14 z-10 shadow-sm">
          <div className="flex items-center gap-4">
            <Link href="/admin/cms/categories">
              <Button variant="ghost" size="icon">
                <ArrowLeft className="h-4 w-4" />
              </Button>
            </Link>
            <div>
              <h1 className="text-xl font-bold">{initialData ? initialData.name : "New Category"}</h1>
          ...
          <div className="flex items-center gap-3">
            {initialData && (
              <a href={`/category/${initialData.slug}`} target="_blank" rel="noreferrer">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="e.g. Content Creation" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Controls the visibility of this category on the site.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
