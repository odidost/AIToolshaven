...
function cleanUp() {
    ...
    // Filter out the broken ones added recently
    // They either have categoryId === 'category-1783781229083' or are newly added. 
    // We can just filter out any tools with this categoryId.
    const initialCount = toolsData.length;
    toolsData = toolsData.filter(t => {
        if (t.publishedData && t.publishedData.category === categoryId) return false;
        if (t.draftData && t.draftData.category === categoryId) return false;
        return true;
    });

    fs.writeFileSync(toolsFilePath, JSON.stringify(toolsData, null, 2));
    console.log(`Removed ${initialCount - toolsData.length} broken tools from tools.json`);
    ...
    // Remove keys like "", "w", "-logo.png", etc. and tools that were added.
    const badKeys = Object.keys(manifestData).filter(k => k.length <= 2 || k.includes('-logo.png'));
    for (const key of badKeys) {
        delete manifestData[key];
    }
    fs.writeFileSync(manifestFilePath, JSON.stringify(manifestData, null, 2));
    console.log(`Cleaned up manifest.json, removed keys: ${badKeys.join(', ')}`);
}
...
async function main() {
    ...
    let toolsData = JSON.parse(fs.readFileSync(toolsFilePath, 'utf8'));
    let manifestData = JSON.parse(fs.readFileSync(manifestFilePath, 'utf8'));
    ...
    for (const tool of toolsList) {
        ...
        console.log(`Processing ${tool.name}... (slug: ${slug})`);
